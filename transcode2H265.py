...
def run_script():
    ...
    parser.add_argument('-v', '--version', action='version', version='3.1.1', help=_("Show program's version number and exit.")) # I need to use this explicit help message here (together with setting add_help=False when creating the parser) to be able to proper translate the version help message (when required). All other messages are translated OK, but not this one. With this edit now everything is OK.
