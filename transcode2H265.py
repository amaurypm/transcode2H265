...
class Video:
    ...
    def __init__(self,filename):
        ...
        self.__ffmpeg_output_postfix='_tmp-transcode2H265'
...
##            self.__sub_exts.append(extra_sub_ext)
    ...
    def transcode(self):
        if self.__transcoding_options_set:
            ...
            cmd_line+=' -c:a aac -ar 48k -b:a 192k -strict experimental -sn -threads {:d} -y \"{}\"'.format(self.__threads, self.__ffmpeg_output)
    ...
    def __get_crop_data(self):
        crop_data=None
        crop_list=[]
        tmp_output_filename=os.path.splitext(self.__in_filename)[0]+'_tmp_transcode2H265-autocrop.mkv'
...
def run_script():
    ...
    parser.add_argument('-v', '--version', action='version', version='3.0.2', help=_("Show program's version number and exit.")) # I need to use this explicit help message here (together with setting add_help=False when creating the parser) to be able to proper translate the version help message (when required). All other messages are translated OK, but not this one. With this edit now everything is OK.
